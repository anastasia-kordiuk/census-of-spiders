...
tbl_spc <- filter(tbl_spc, spec %in% top100)
...
tmp <- mclapply(
    ...
    1:nrow(models2), function(x){
        ...
        res <- input %>% 
            ...
            mutate(mid_pred_2 = mid_pred^2, .after = mid_pred) %>% 
            lm(formula = as.formula("n_rec ~ mid_pred + mid_pred_2"), data = .)
        sm_res <- summary(res)
...
# замена названий факторов на русские
...
mod_fam <- models_selected %>% 
    ...
    mutate_at(c("A_pval", "B_pval", "shapiro_test", "A"), ~round(.x, 3)) %>% 
...
df_predicted <- df_tmp %>% 
    ...
    lapply(function(xx){
        ...
        mutate(d, n_rec = predict(xx$models[[1]], newdata = d))
    }) %>% 
    map_dfr(rbind, .id = "model") %>% 
    filter(n_rec >= 0) %>% 
    separate(model, "Family", sep = " ~ ", extra = "drop", remove = F)

graph_fam <- ggplot(df_observed, aes(mid_pred, n_rec, color = Family)) + 
    geom_point() + 
    ...
    facet_wrap(~model, 
               ...
               scales = "free") +
    labs(x = NULL, y = "Количество находок", 
         color = "Семейства") + 
    theme_bw() + 
...
ggsave("plotfam.pdf", plot = graph_fam, device = cairo_pdf, width = 9, height = 8.5)
...
models3 <- expand_grid(
    ...
    mutate(frm =   paste0("n_rec ~ ", fct, " + ", fct, "_2"))
...
ggplot(df_observed_sp, aes(mid_pred, n_rec, color = Species)) + 
    geom_point() + 
...
graph_sp <- ggplot(df_observed_sp, aes(mid_pred, n_rec, color = Species)) + 
    geom_point() + 
    geom_line(data = df_predicted_sp) + 
    facet_wrap(~model, 
               labeller = labeller(model = function(x) {
...
ggsave("plotsp.pdf", plot = graph_sp, device = cairo_pdf, height = 8.5, width = 9)